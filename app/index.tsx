...
import { decideBillingReturn } from "../src/utils/billingGate";
...
export default function Index() {
  ...
  // Stripe redirects back to APP_URL with ?billing-success=1&session_id=… (paid) or
  // ?billing-cancel=1 (backed out). On the success return we briefly poll for the webhook-confirmed
  // status (Stripe's webhook can lag the redirect by a couple of seconds; without polling the user
  // lands on the paywall right after paying and has no idea why). On cancel we keep the paywall up
  // but pick mode based on what the user actually has — "pending" (mid-signup) stays in signup
  // copy, "expired" gets the post-trial copy. Query params are cleared so this doesn't re-trigger.
  useEffect(() => {
    if (typeof window === "undefined" || !business?.code || business.code === "DEMO") return;
    const decision = decideBillingReturn(window.location.search);
    if (decision === "ignore") return;
    (async () => {
      if (decision === "success-check") {
        // Brief poll — webhook (proxy.js:checkout.session.completed) usually fires within ~1s of
        // the redirect, but Stripe's docs warn it can take longer under load. 8 tries × 2s = 16s.
        for (let i = 0; i < 8; i++) {
          try {
            ...
            if (st.status === "active" || st.status === "veraa" || st.status === "trialing") {
              const updated: Business = {
                ...business,
                subscriptionStatus: st.status as Business["subscriptionStatus"],
                ...(st.status === "trialing" && !business.trialStartedAt ? { trialStartedAt: Date.now() } : {}),
              };
              setBusiness(updated);
        ...
        // cancel — leave the user on the paywall in whichever mode matches their actual state.
        setPaywallCancelled(true);
        setPaywallMode(business.subscriptionStatus === "pending" ? "signup" : "expired");
        setScreen("paywall");
      }
