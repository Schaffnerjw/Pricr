...
export function HistoryScreen({ business, currentUser, onBack, onNewQuote }: {
  ...
  const setStatus = async (id: string, status: QuoteStatus) => {
    ...
    await updateQuote(business.code, id, { status });
  };
  const remove = async (id: string) => {
    setMenuId(null);
    setQuotes(qs => qs.filter(q => q.id !== id));
    await deleteQuoteFromStorage(business.code, id);
  };
    ...
    <SafeAreaView style={s.container}>
      ...
      {loading ? (
        ...
        <View style={s.centered}>
          ...
          <Text style={s.h2}>No quotes yet</Text>
          <Text style={[s.body, { textAlign: "center", marginVertical: 12 }]}>No quotes yet — open your tool to get started.</Text>
          <TouchableOpacity style={[s.btn, { backgroundColor: brand.primaryColor }]} onPress={onNewQuote}>
            <Text style={s.btnText}>Open Quote Tool</Text>
          </TouchableOpacity>
        </View>
      ) : (
        <ScrollView contentContainerStyle={{ padding: 20, gap: 12 }} keyboardShouldPersistTaps="handled">
          ...
          <View style={s.histSummaryRow}>
            ...
            <Text style={[s.histSummaryItem, { color: B.gray2 }]}>Open: {open}</Text>
          </View>

          {/* Search */}
          <View style={s.histSearch}>
            <Feather name="search" size={16} color={B.gray3} />
            <TextInput style={s.histSearchInput} placeholder="Search by customer name" placeholderTextColor={B.gray3} value={query} onChangeText={setQuery} autoCapitalize="none" />
            {query ? <TouchableOpacity onPress={() => setQuery("")}><Feather name="x" size={16} color={B.gray3} /></TouchableOpacity> : null}
          </View>

          {filtered.length === 0 ? (
            <Text style={[s.emptyText, { textAlign: "center", marginTop: 20 }]}>No matches for “{debounced}”.</Text>
          ) : filtered.map(q => (
            <TouchableOpacity key={q.id} style={s.historyCard} onLongPress={() => setMenuId(q.id)} delayLongPress={250}>
              <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start" }}>
                <View style={{ flex: 1 }}>
                  <View style={s.histRowTop}>
                    ...
                    {q.isSample && (
                      <View style={{ backgroundColor: B.border, borderRadius: 6, paddingHorizontal: 6, paddingVertical: 2 }}>
                        <Text style={{ fontSize: 10, fontWeight: "700", letterSpacing: 0.5, color: B.gray2, fontFamily: "DMSans_700Bold" }}>SAMPLE</Text>
                      </View>
                    )}
                  </View>
                  <Text style={[s.historyMeta, { marginTop: 4 }]}>{q.trade} · {formatDate(q.timestamp)}{currentUser.role === "admin" && q.repName ? ` · ${q.repName}` : ""}</Text>
                </View>
                <Text style={[s.historyTotal, { color: brand.primaryColor }]}>{formatMoney(q.total || 0)}</Text>
              </View>
              <Text style={{ color: B.gray3, fontSize: 11, marginTop: 6, fontFamily: "DMSans_400Regular" }}>Long-press to mark won/lost or delete</Text>
            </TouchableOpacity>
          ))}
        </ScrollView>
      )}
